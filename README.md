...
四个片单都输出 TMDB ID 和类型，封面统一使用 TMDB 图片服务生成。日番同时读取 TMDB、Bangumi 和 AniList，BGM/AniList 条目通过标题、年份和地区匹配到 TMDB 后参与排序；假面骑士和超级战队使用正剧白名单，从 TMDB 查询并排除电影、剧场版、特别篇和衍生剧。综艺从 TMDB 获取最新季和分集日期。
