...
- 片单 1：从 TMDB 筛选中国大陆真人秀/脱口秀类型；只取当前年度已上线的最新普通季，排除未来节目、MV、纯享版和特别篇；每日重建近 45 天有正片更新或未来 30 天已有正片排期的节目，当天新季上线或有正式正片更新的节目置顶，其他符合条件的节目继续保留，最多 50 部。
- 片单 2：同时订阅 TMDB、Bangumi（BGM）和 AniList 的今年日本动画，三源合并并统一映射为 TMDB ID，按今年内最新一季的首播时间从新到旧输出；既包含今年首播的新番，也包含挂在老剧集条目下的今年续作季（如 2026 年开播的芙莉莲第二季、无职转生第三季），并自动合并 TMDB 的重复条目，片单名称永久为“厕纸”。
- 片单 3：从 TMDB 获取假面骑士 2000 年至今的正剧 TV 系列，按首播时间从新到旧。
- 片单 4：从 TMDB 获取东映超级战队从 1975 年《秘密战队五连者》至今的正剧 TV 系列，按首播时间从新到旧。

四个片单都输出 TMDB ID 和类型，封面统一使用 TMDB 图片服务生成。日番同时读取 TMDB、Bangumi 和 AniList，BGM/AniList 条目通过标题、年份和地区匹配到 TMDB 后参与排序；续作季按其在 TMDB 上所属剧集的最新已开播季首播日期参与排序，日期在未来、尚未开播的条目不会进入片单；同一部番在 TMDB 存在多个条目时按 IMDb/TVDB ID 和标题自动合并。假面骑士和超级战队使用正剧白名单，从 TMDB 查询并排除电影、剧场版、特别篇和衍生剧。综艺从 TMDB 获取最新普通季和分集日期；每日同时扫描热度池和按首播日期倒序的候选池，避免当天上线的新节目因热度不足漏选。

## 手工修正重复条目（mapping.json）

TMDB 上偶尔存在同一部动画的重复条目。自动去重之外，可以在 `mapping.json` 里把重复条目手工指向主条目（对日番片单生效）：
...
含义是把 TMDB 326191 视为 327843 的重复条目，片单里只保留 327843。键是被重定向的 TMDB ID，值是主条目 ID。
...
综艺片单使用 TMDB Discover TV、详情、季和分集接口，筛选中国大陆综艺并按正片日期排序。日番片单订阅 TMDB Discover TV、Bangumi Subject Search 和 AniList GraphQL Media；筛选本年度已经上线的日本动画（含今年开播的续作季，取剧集在 TMDB 上最新已开播季的首播日期），按 IMDb/TVDB ID、标题去重并映射为 TMDB TV ID 后，按今年内最新季首播时间从新到旧输出，上限 500 部。假面骑士和超级战队使用固定的正剧系列白名单从 TMDB Search TV 查询，不录入电影、特别篇、剧场版或衍生作品。
